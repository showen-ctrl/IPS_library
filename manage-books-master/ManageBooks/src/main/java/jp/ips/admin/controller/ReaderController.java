...
import jp.ips.admin.pojo.*;
import jp.ips.admin.service.BookService;
...
public class ReaderController {
    ...
    public String listBookListById(
        ...
        if (rr.isSuccess()) {
            pw.print("<script>alert('预约成功');window.location.href='/admin/booklist';</script>");
        } else {
            pw.print("<script>alert('预约失败,请重新预约!');window.location.href='/admin/booklist';</script>");
        }
        List<BookList> list = bookService.getlist();
        model.addAttribute("list", list);
        return "user_booklist";
